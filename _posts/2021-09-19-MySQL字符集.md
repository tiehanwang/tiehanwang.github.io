...
​	比方说我们来自定义一个名称为`x`的字符集，它包含的字符范围和编码规则如下：
...
有了`x`字符集，我们就可以用二进制形式表示一些字符串了，下边是一些字符串用`x`字符集编码后的二进制表示：

```
'bA' -> 0000001000000011  (十六进制：0x0203)
'baB' -> 000000100000000100000100  (十六进制：0x020104)
'cd' -> 无法表示，字符集x不包含字符'c'和'd'
```

## 比较规则简介

确定字符集的字符范围及编码规则后 要考虑如何比较两个字符大小。

最简单的做法就是直接比较这两个字符对应的二进制编码的大小。

比方说字符`'a'`的编码为`0x01`，字符`'b'`的编码为`0x02`，所以`'a'`小于`'b'`，这种简单的比较规则也可以被称为二进制比较规则，英文名为`binary collation`。

但是我们往往有更多的需求 比如某种比较情况下需要`'a'`和`'A'`是相等的，这种情况需要设置额外的规则

也就是说<span style="color:red">**同一种字符集可以有多种比较规则**</span>
...
其中的`Default collation`列表示这种字符集中一种默认的`比较规则`。大家注意返回结果中的最后一列`Maxlen`，它代表该种字符集表示一个字符最多需要几个字节。为了让大家的印象更深刻，我把几个常用到的字符集的`Maxlen`列摘抄下来，大家务必记住：
...
![](https://raw.githubusercontent.com/tiehanwang/tiehanwang.github.io/master/img/doc-mysql-character.PNG)
...
我们把视角再次聚焦到`比较规则`，`比较规则`的作用通常体现比较字符串大小的表达式以及对某个字符串列进行排序中，所以有时候也称为`排序规则`。比方说表`t`的列`col`使用的字符集是`gbk`，使用的比较规则是`gbk_chinese_ci`，我们向里边插入几条记录：
